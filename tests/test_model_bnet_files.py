...
Each test case:
1. Discovers an example directory with model.bnet, observation files, and output.txt
...
def discover_examples():
    ...
    - model.bnet
    - at least one observation .lp file (not model.bnet)
    - output.txt
    """
        ...
        if category_dir.is_dir():
            for example_dir in sorted(category_dir.iterdir()):
                if example_dir.is_dir():
                    model_bnet = example_dir / "model.bnet"
                    output_txt = example_dir / "output.txt"

                    if model_bnet.exists() and output_txt.exists():
                        obs_files = [
...
def test_example_output(example_dir: Path, venv_python: str):
    ...
    """
    model_file = str(example_dir / "model.bnet")
